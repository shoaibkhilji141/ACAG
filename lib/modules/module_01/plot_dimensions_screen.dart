...
class _PlotDimensionsScreenState extends State<PlotDimensionsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final row = await ProjectService.getPlotDimensions(project.id);
      if (!mounted) return;
      if (row != null) {
        final unit = row['unit'] as String? ?? 'feet';
        final zone = row['geographic_zone'] as String? ?? 'punjab_plains';
        _unitIndex = _unitKeys.indexOf(unit).clamp(0, _unitKeys.length - 1);
        _zoneIndex = _zoneKeys.indexOf(zone).clamp(0, _zoneKeys.length - 1);
        _lengthController.text = _fmt(row['length']);
        _widthController.text = _fmt(row['width']);
      }
    } finally {
  ...
  Future<void> _continue() async {
    ...
    if (length == null || width == null || length <= 0 || width <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Enter valid plot length and width')),
      );
      return;
    }
    ...
    try {
      await ProjectService.savePlotDimensions(
        projectCodeOrId: project.id,
        unit: _unitKeys[_unitIndex],
        length: length,
        width: width,
        totalArea: length * width,
        geographicZone: _zoneKeys[_zoneIndex],
      );
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return StitchFlowScaffold(
      ...
      bottomLabel: _saving ? 'Saving…' : 'Continue to Room Requirements',
